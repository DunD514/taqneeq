...
class PaymentEvent:
    ...
    # Optional (reality-grounded, cost-aware, retry-aware)
    merchant_id: Optional[str] = None
    total_attempts: Optional[int] = None  # 1 + retries for attempt-level modeling
...
class WindowMetrics:
    ...
    sample_count: int
    # Optional extensions (merchant-aware, cost-aware, attempt amplification)
    success_rate_by_merchant: dict[str, float] = field(default_factory=dict)
    attempt_amplification_by_merchant: dict[str, float] = field(default_factory=dict)
    average_cost_by_merchant: dict[str, float] = field(default_factory=dict)
...
class Action:
    ...
    reason: str = ""
    requires_human_approval: bool = False  # if True, executor must not auto-execute
