...
def _heuristic_hypothesis(metrics: WindowMetrics) -> Hypothesis:
    ...
    """
    # Low sample count check
    if metrics.sample_count is not None and metrics.sample_count < 50:
        return Hypothesis(
             cause="INSUFFICIENT_SIGNAL",
             confidence=1.0, # High confidence that we have no signal
             evidence=f"Sample count {metrics.sample_count} < 50; insufficient data",
             source="heuristic",
        )
    ...
    # If confidence is too low, explicit unsure
    if confidence > 0 and confidence < 0.5:
         cause = "UNCERTAIN"
         evidence = "Signals conflicting or weak. " + evidence
...
def reason(metrics: WindowMetrics) -> Hypothesis:
    ...
    """
    # Sanity check: if very low samples, don't even bother LLM
    if metrics.sample_count is not None and metrics.sample_count < 50:
