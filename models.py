...
class PaymentEvent:
    ...
    # ---------------- ADDITIVE FIELDS (Plan A) ----------------
    merchant_id: Optional[str] = None

    total_attempts: Optional[int] = None
    retry_amplification_factor: Optional[float] = None
...
class WindowMetrics:
    ...
    # ---------------- ADDITIVE MERCHANT METRICS (Plan A) ----------------
    ...
    # Window-level cost/attempt (for real-time KPIs and risk flags)
    attempt_amplification: Optional[float] = None  # avg attempts per txn
    average_estimated_cost: Optional[float] = None   # avg cost per txn


@dataclass
class Hypothesis:
    """Structured hypothesis from the reasoner (LLM or fallback). Includes uncertainty for real-time decision timing."""
    cause: str
    confidence: float
    evidence: str
    source: str = "heuristic"  # "llm" | "heuristic"
    uncertainty: float = 0.0  # 0-1; higher = more uncertain; influences when to trigger reason/decide
...
class Action:
    ...
    target: Optional[str] = None  # issuer | flow | merchant_id
...
class PendingApproval:
    """State for an action waiting for human approval."""
    action: Action
    risk_score: float
    reason: str
    timestamp: float
